...
src/
├── application/
│   ├── RegisterInput                    // パスワード登録入力 DTO
│   └── RegisterResult                   // 登録結果 DTO
├── domain/
│   ├── User                             // アプリケーションユーザー
│   ├── AuthenticatedUser                // 認証成功後の正規化表現
│   └── AuthProviderCredential           // email + provider の認証情報
├── service/
│   └── UserRegistrationService          // 登録のオーケストレーション
├── repository/
│   ├── UserDBResistory                  // AuthenticatedUser → User 変換と DB 保存
│   └── Database                         // DB モック
├── email/
│   └── EmailClient                      // メール送信モック
└── oauth/
    ├── OAuthProvider                    // ベンダー共通契約：code → OAuthUserInfo
    ├── OAuthException                   // 外部プロバイダ起因の失敗
    └── github/
        └── GitHubOAuthProvider          // GitHub 実装
```
...
- **domain model の分離**：`User` や `AuthenticatedUser` などを `UserRegistrationService` の inner class にせず、独立した domain / DTO class として定義した。
- **責務別 package 構成**：`domain`、`application`、`service`、`repository`、`email`、`oauth` に分け、クラスの所属と依存方向を読み取りやすくした。
- **テスト容易性**：`UserDBResistory` に User の組み立てと保存を切り出し、`Database`/`EmailClient`/`OAuthProvider` を差し替え可能にした。
