...
入力(方法ごとに異なる) ──認証──▶ AuthenticatedUser(共通の中間表現) ──共通パイプライン──▶ 結果
 ...
 ・GitHub OAuth                   externalId / passwordHash         DB保存
...
- **ベンダー差異の吸収**：`OAuthUserInfo` で `email/name/externalId` に正規化し、後続がプロバイダを意識しないようにした。
...
- **アカウント連携**：OAuth のメールが既存ユーザーと一致した場合、現状はエラー。本来は `(provider, externalId)` をキーに既存アカウントへ連携 or ログインに分岐したい。
