...
### [ADR-008] src 配下を責務別 package に分割した

**日時**：2026-06-27

**状況**：
`src` 直下に service、domain model、DTO、repository、OAuth 実装が並んでいた。クラス数が増えたことで、どの層の責務かがファイル配置から読み取りづらくなっていた。

**決定**：
`application`、`domain`、`service`、`repository`、`email`、`oauth` に package を分けた。GitHub 固有実装は `oauth.github` に置いた。あわせて `Database` と `EmailClient` も `UserRegistrationService` の inner class から外し、repository / email 側のモックとして独立させた。

**理由**：
ファイル配置と責務を一致させることで、今後 Google / LINE 追加、DB 実装差し替え、メール送信差し替えを行う場所が明確になる。service 層は登録フローの制御だけを担う構造にできる。

**トレードオフ**：
小規模なスターターコードとしてはディレクトリ数が増えるが、既に domain model と repository を分離しているため、構成もそれに合わせた方が保守しやすい。
